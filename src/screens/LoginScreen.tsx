...
export default function LoginScreen() {
  ...
  useEffect(() => {
    if (!settings.isSetup) setStep('create');
    else setStep('enter');
  }, [settings.isSetup]);
  ...
  const handleNum = (n: string) => {
    ...
    if (step === 'enter' || step === 'create') {
      ...
      if (confirmPin.length < 6) setConfirmPin(p => p + n);
    }
  };
  ...
  useEffect(() => {
    if (step === 'enter' && pin.length === 6) {
      setTimeout(() => {
        const ok = login(pin);
        if (!ok) {
          triggerShake('PIN salah, coba lagi');
          setPin('');
        }
      }, 150);
    }
  ...
  const handleNameSubmit = () => {
    if (!name.trim()) { toast.error('Masukkan nama Anda'); return; }
    setStep('gasurl');
  };

  const handleGasSubmit = (skip = false) => {
    saveSettings({ pin, userName: name.trim(), gasUrl: skip ? '' : gasUrl.trim(), isSetup: true, currency: 'IDR' });
    login(pin);
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-violet-600 via-purple-700 to-indigo-800 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm animate-scaleIn">
          <div className="text-center mb-8">
            <div className="inline-flex w-20 h-20 bg-white/15 rounded-3xl items-center justify-center mb-4 text-4xl">
              💰
            </div>
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20">
            ...
            <input
              ...
              autoFocus
              className="w-full bg-white/15 border border-white/25 text-white placeholder-white/40 rounded-2xl px-4 py-3.5 outline-none focus:border-white/60 transition text-sm font-medium mb-4"
            />
            <button
              onClick={handleNameSubmit}
              className="w-full bg-white text-violet-700 font-bold rounded-2xl py-3.5 text-sm active:scale-95 transition-transform shadow-lg"
            >
              Lanjutkan →
            </button>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20 space-y-4">
            <div className="bg-white/10 rounded-2xl p-4">
              ...
              <ol className="text-white/70 text-xs space-y-1 list-decimal list-inside">
                ...
                <li>Paste kode dari menu Pengaturan</li>
            ...
            <input
              ...
              className="w-full bg-white/15 border border-white/25 text-white placeholder-white/40 rounded-2xl px-4 py-3.5 outline-none focus:border-white/60 transition text-xs font-medium"
            />
            <button
              onClick={() => handleGasSubmit(false)}
              className="w-full bg-white text-violet-700 font-bold rounded-2xl py-3.5 text-sm active:scale-95 transition-transform shadow-lg"
            >
              Mulai Sekarang 🎉
            </button>
            <button
              onClick={() => handleGasSubmit(true)}
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-600 via-purple-700 to-indigo-800 flex flex-col items-center justify-center p-6 select-none">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex w-20 h-20 bg-white/15 rounded-3xl items-center justify-center mb-4 text-4xl shadow-lg">
            💰
          </div>
          <h1 className="text-3xl font-black text-white tracking-tight">FinTrack</h1>
        ...
        {/* Step label */}
        <p className="text-center text-white/80 font-semibold text-base mb-8">
          {stepInfo[step].title}
        </p>
        ...
        {/* Error message */}
        {errorMsg && (
          <p className="text-center text-red-300 text-sm font-medium mb-6 -mt-4">{errorMsg}</p>
        )}
          ...
          {['1','2','3','4','5','6','7','8','9','','0','del'].map((n, idx) => {
            ...
            if (n === 'del') return (
              <button
                key={idx}
                onClick={handleDel}
                className="h-[70px] flex items-center justify-center rounded-2xl bg-white/10 active:bg-white/25 transition-colors active:scale-90 transition-transform"
              >
            ...
            return (
              <button
                key={idx}
                onClick={() => handleNum(n)}
                className="h-[70px] text-white text-2xl font-bold rounded-2xl bg-white/10 hover:bg-white/15 active:bg-white/30 transition-colors active:scale-90 transition-transform"
              >
                {n}
              </button>
            );
        ...
        {step === 'create' && (
          <p className="text-center text-white/40 text-xs mt-8">
            Jangan berbagi PIN dengan siapapun
          </p>
        )}
      </div>
    </div>

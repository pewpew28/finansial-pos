...
export default function DashboardScreen() {
  ...
  const WALLET_ICON_LIST: Record<string, string> = {
    wallet: '👛', card: '💳', bank: '🏦', piggy: '🐷',
    bag: '💼', phone: '📱', star: '⭐', dollar: '💵', gem: '💎', chart: '📈',
  };
    ...
    <div className="pb-4">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-violet-600 via-purple-700 to-indigo-700 px-5 pt-14 pb-28">
        ...
        <div className="relative flex items-start justify-between">
          ...
          <button
            onClick={syncData}
            className={`w-9 h-9 bg-white/15 rounded-xl flex items-center justify-center active:scale-90 transition-transform ${isSyncing ? 'animate-spin' : ''}`}
          >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-emerald-50 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-gray-500 font-medium">Pemasukan</span>
              </div>
            ...
            <div className="bg-red-50 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-7 h-7 bg-red-500 rounded-xl flex items-center justify-center">
                  <TrendingDown size={13} className="text-white" />
                </div>
                <span className="text-xs text-gray-500 font-medium">Pengeluaran</span>
              </div>
      ...
      <div className="px-4 mt-4 space-y-4">
        ...
        {wallets.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-bold text-gray-800">Dompet Saya</h2>
              <span className="text-xs text-violet-600 font-semibold">{wallets.length} dompet</span>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-none -mx-4 px-4">
              {wallets.map((w, idx) => (
                <div
                  key={w.id}
                  className="flex-shrink-0 w-44 rounded-2xl p-4 text-white relative overflow-hidden"
                ...
                >
                  ...
                  {idx === 0 && (
                    <div className="absolute top-3 right-3">
                      <span className="text-[9px] bg-white/25 text-white px-1.5 py-0.5 rounded-full font-bold">UTAMA</span>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="grid grid-cols-3 gap-2.5">
          <div className="bg-white rounded-2xl p-3.5 shadow-sm border border-gray-100 text-center">
            <p className="text-lg font-black text-blue-500">🤝</p>
            <p className="text-[10px] text-gray-400 font-medium mt-1">Piutang</p>
            <p className="font-black text-blue-600 text-[11px] mt-0.5">{formatCurrency(totalReceivable)}</p>
          </div>
          <div className="bg-white rounded-2xl p-3.5 shadow-sm border border-gray-100 text-center">
            <p className="text-lg font-black text-orange-500">💸</p>
            <p className="text-[10px] text-gray-400 font-medium mt-1">Utang</p>
            <p className="font-black text-orange-600 text-[11px] mt-0.5">{formatCurrency(totalPayable)}</p>
          </div>
          <div className="bg-white rounded-2xl p-3.5 shadow-sm border border-gray-100 text-center">
            <p className="text-lg font-black text-violet-500">💳</p>
            <p className="text-[10px] text-gray-400 font-medium mt-1">Cicilan/bln</p>
            <p className="font-black text-violet-600 text-[11px] mt-0.5">{formatCurrency(totalMonthlyInstall)}</p>
          </div>
        </div>
        ...
        {transactions.length > 0 && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-4">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-sm font-bold text-gray-800">Tren 7 Hari</h2>
              <div className="flex items-center gap-3 text-[10px] text-gray-400">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block" />Masuk</span>
            ...
            <ResponsiveContainer width="100%" height={130}>
              <AreaChart data={chartData} margin={{ top: 8, right: 0, left: -28, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ borderRadius: 12, border: 'none', boxShadow: '0 4px 24px rgba(0,0,0,0.12)', fontSize: 11, background: '#1e1b4b', color: '#fff' }}
            ...
            <div className="flex items-center gap-4">
              <div className="flex-shrink-0">
                <PieChart width={96} height={96}>
                  <Pie data={categoryData} cx={43} cy={43} innerRadius={26} outerRadius={44} paddingAngle={3} dataKey="value">
                    ...
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="w-full h-1 bg-gray-100 rounded-full mt-0.5">
                          <div className="h-1 rounded-full" style={{ width: `${pct}%`, background: PIE_COLORS[i % PIE_COLORS.length] }} />
                        </div>
                      </div>
                    </div>
                  );
              ...
              {recentTx.map(tx => {
                ...
                return (
                  <div key={tx.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-gray-800 truncate">{tx.description || tx.category}</p>
                      <p className="text-[10px] text-gray-400">{formatDateShort(tx.date)} · {wallet?.name ?? '-'}</p>
                    </div>
                    <p className={`text-xs font-black flex-shrink-0 ${tx.type === 'income' ? 'text-emerald-500' : 'text-red-500'}`}>
        ...
        {!hasData && (
          <div className="text-center py-8 text-gray-400">
            ...
            <p className="font-bold text-gray-600">Mulai kelola keuangan Anda</p>
            <p className="text-sm mt-1">Tambahkan dompet untuk memulai</p>
          </div>
        )}
      </div>
    </div>

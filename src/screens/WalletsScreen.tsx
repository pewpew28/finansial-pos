...
export default function WalletsScreen() {
  ...
  const openAdd = () => {
    setEditId(null);
    setForm({ name: '', balance: '', color: WALLET_COLORS[0], icon: 'wallet' });
    setShowModal(true);
  };
  ...
  const openEdit = (id: string) => {
    ...
    if (!w) return;
    setEditId(id);
    setForm({ name: w.name, balance: String(w.balance), color: w.color, icon: w.icon });
    setShowModal(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (editId) {
      await updateWallet(editId, { name: form.name.trim(), balance: bal, color: form.color, icon: form.icon });
    } else {
      await addWallet({ name: form.name.trim(), balance: bal, color: form.color, icon: form.icon });
    }
    setShowModal(false);
  };
    ...
    <div className="pb-4">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-violet-600 to-indigo-700 px-5 pt-14 pb-24">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-white text-xl font-black">Dompet</h1>
            <p className="text-white/60 text-xs mt-0.5">{wallets.length} dompet aktif</p>
          </div>
          <button
            onClick={openAdd}
      ...
      <div className="px-4 -mt-12 space-y-3 relative z-10">
        {wallets.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-xl p-10 text-center text-gray-400">
            ...
            <p className="font-bold text-gray-600">Belum ada dompet</p>
            <p className="text-sm mt-1">Ketuk + untuk menambah dompet</p>
          </div>
        ) : (
          wallets.map((w, idx) => {
                ...
                <div className="px-5 py-3 flex items-center gap-4 bg-gray-50/50 border-t border-gray-100">
                  <div className="flex items-center gap-1.5 flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-black text-emerald-600">{formatCurrency(stats.income)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-black text-red-500">{formatCurrency(stats.expense)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 ml-auto">
                    ...
                    >
                      <Pencil size={13} style={{ color: w.color }} />
                    </button>
                    <button
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editId ? 'Edit Dompet' : 'Tambah Dompet'}>
        <div className="space-y-4">
          ...
          <div className="rounded-2xl p-4 flex items-center gap-3" style={{ background: `${form.color}15` }}>
            <div className="w-12 h-12 rounded-2xl text-2xl flex items-center justify-center shadow-sm" style={{ background: form.color }}>
              {getEmoji(form.icon)}
            </div>
            <div>
              <p className="font-black text-gray-900">{form.name || 'Nama Dompet'}</p>
              <p className="text-sm text-gray-500">Rp {form.balance || '0'}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative mt-1.5">
              ...
              <input
                ...
                inputMode="numeric"
                value={form.balance}
                onChange={e => setForm(f => ({ ...f, balance: fmt(e.target.value) }))}
                placeholder="0"
            ...
            <div className="flex flex-wrap gap-2.5">
              {WALLET_COLORS.map(c => (
                <button
                  ...
                  className={`w-9 h-9 rounded-xl transition-all active:scale-90 ${form.color === c ? 'ring-2 ring-offset-2 ring-gray-400 scale-110' : ''}`}
                  style={{ background: c }}
                />
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {WALLET_ICONS.map(ic => (
                <button
                  ...
                  style={form.icon === ic.id ? { background: `${form.color}25` } : {}}
                >
                  {ic.emoji}
                </button>
              ))}
          ...
          <button
            ...
            disabled={!form.name.trim() || !form.balance}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (deleteId) deleteWallet(deleteId); }}
        title="Hapus Dompet?"
        message="Dompet ini akan dihapus permanen. Data transaksi terkait tetap tersimpan."
      />
    </div>
  );
}

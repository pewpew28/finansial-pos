...
export default function TransactionsScreen() {
  ...
  return (
    <div className="pb-4">
      ...
      {/* Search bar */}
      {showSearch && (
        <div className="px-4 pt-3 animate-scaleIn">
        ...
        {(['all', 'income', 'expense'] as FilterType[]).map(f => (
          <button
            ...
            className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all active:scale-95 ${
              filterType === f ? filterColors[f] : 'bg-white text-gray-500 border border-gray-200'
            }`}
          >
            {filterLabels[f]}
          </button>
                      ...
                      <div key={tx.id} className={`flex items-center gap-3 px-4 py-3 ${idx < txs.length - 1 ? 'border-b border-gray-50' : ''}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            {wallet && (
                              <span className="text-[10px] text-gray-400">· {wallet.name}</span>
                            )}
                          </div>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          <button
                            ...
                            className="w-7 h-7 rounded-lg bg-gray-50 flex items-center justify-center active:scale-90 transition-transform"
                          >
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Tambah Transaksi">
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={!form.amount || !form.walletId || !form.category}
            className={`w-full py-4 rounded-2xl font-black text-sm transition-all active:scale-95 ${
              !form.amount || !form.walletId || !form.category
      ...
      <ConfirmDialog
        ...
        message="Transaksi akan dihapus dan saldo dompet akan disesuaikan kembali."
      />
    </div>
  );
}
